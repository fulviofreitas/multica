...
// TestInboundFromMessageCreate_MentionsArray exercises the authoritative
// "mentions" array signal added to fix the reply-with-ping-enabled bug: a
// reply with Discord's default ping-the-replied-to-author behavior adds the
// bot to "mentions" WITHOUT ever inserting a "<@id>" token into Content, so
// mentionsBotID's content regex alone cannot see it. See identify.go's
// MessageCreateEvent.Mentions doc comment.
func TestInboundFromMessageCreate_MentionsArray(t *testing.T) {
	tests := []struct {
		name          string
		evt           MessageCreateEvent
		wantAddressed bool
		...
		{
			// THE BUG THIS TEST CATCHES: before the fix, inboundFromMessageCreate
			// only ever consulted Content via mentionsBotID's regex, so this case
			// (bot in Mentions, no "<@id>" token in Content — exactly what Discord
			// sends for a plain reply-with-ping) computed AddressedToBot=false even
			// though Discord itself considers the bot mentioned (and, per Discord's
			// documented MESSAGE_CONTENT exemption, sent us full content on that
			// basis). Confirmed failing against the pre-fix code; see task report.
			name: "bot present in mentions array, no <@id> token in content, is addressed (reply-with-ping case)",
			evt: MessageCreateEvent{
				ID:        "30",
				ChannelID: "guild-chan",
				GuildID:   "guild-1",
				...
				Author:    MessageAuthor{ID: "42", Username: "someone", Bot: false},
				Mentions:  []MentionedUser{{ID: testBotID}},
			},
			wantAddressed: true,
			wantText:      "thanks for the help",
		},
		{
			name: "bot in mentions AND content token: addressed, no double-count, token still stripped",
			evt: MessageCreateEvent{
				ID:        "31",
				ChannelID: "guild-chan",
				GuildID:   "guild-1",
				Content:   "<@" + testBotID + "> thanks for the help",
				Author:    MessageAuthor{ID: "42", Username: "someone", Bot: false},
				Mentions:  []MentionedUser{{ID: testBotID}},
			},
			wantAddressed: true,
		...
		{
			name: "another user in mentions, not our bot: not addressed via the array",
			evt: MessageCreateEvent{
				ID:        "32",
				ChannelID: "guild-chan",
				GuildID:   "guild-1",
				...
				Author:    MessageAuthor{ID: "42", Username: "someone", Bot: false},
				Mentions:  []MentionedUser{{ID: "some-other-user"}},
			},
			wantAddressed: false,
			wantText:      "hey check this out",
		},
		{
			name: "empty/absent mentions array falls back to content regex, existing behavior unchanged",
			evt: MessageCreateEvent{
				ID:        "33",
				ChannelID: "guild-chan",
				GuildID:   "guild-1",
...
// TestInboundFromMessageCreate_ReplyToBotDegraded covers the genuinely
// degraded case: the message is addressed ONLY via repliedToBot (a reply to
// our bot with the ping DISABLED), the bot is NOT in Mentions, and Discord
// therefore blanks Content to "" (the MESSAGE_CONTENT exemption does not
// apply, since none of the four documented conditions hold). We choose to
// treat this as NOT addressed (AddressedToBot=false) rather than deliver an
// empty-text "addressed" message to the agent:
//
//   - Delivering it (Option A) hands the agent a message with no text and no
//     way to recover one (no REST call, per ReplyToAuthorID's own doc on not
//     resolving unknowns) — at best a wasted agent turn, at worst a
//     confusing empty reply back to the user.
//   - Treating it as unaddressed (Option B, chosen) means AddressedToBot is
//     false, so the Router drops and audits it (DropReasonNotAddressedInGroup)
//     — leaving an operator-visible trace that "the bot saw a reply it could
//     not read" instead of silently misbehaving. The user who replied with
//     the ping off gets no response, which is the same experience as any
//     other message the bot doesn't consider addressed to it — arguably the
//     more honest outcome, since the bot genuinely does not have their text.
//
// This does NOT change the non-degraded repliedToBot case (ping irrelevant,
// content present) covered by TestInboundFromMessageCreate_ReplyToBot.
func TestInboundFromMessageCreate_ReplyToBotDegraded(t *testing.T) {
	evt := MessageCreateEvent{
		ID:               "40",
		ChannelID:        "guild-chan",
		GuildID:          "guild-1",
		Content:          "", // Discord blanks content: no exemption applies
		Author:           MessageAuthor{ID: "42", Username: "someone", Bot: false},
		ReplyToMessageID: "parent-40",
		ReplyToAuthorID:  testBotID,
		Mentions:         nil, // ping disabled: bot not added to mentions
	}
	...
	if msg.AddressedToBot {
		t.Error("AddressedToBot = true, want false: a reply-to-bot with the ping disabled and empty content must not be treated as addressed (see doc comment above)")
	}
	if msg.ReplyTo == nil || msg.ReplyTo.MessageID != "parent-40" {
		t.Errorf("ReplyTo = %+v, want MessageID %q (ReplyTo must still be populated for context even when not addressed)", msg.ReplyTo, "parent-40")
	}
}
