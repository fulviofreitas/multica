...
// MentionedUser is one entry of a MESSAGE_CREATE's "mentions" array: the
// user objects Discord itself resolved as mentioned by this message. Only
// ID is needed today (matching it against our own bot id), but this is kept
// as its own named type rather than a bare []string so a later subtask that
// needs Username/Bot for some other purpose does not have to re-decode.
...
type MessageCreateEvent struct {
	...
	// addressing signal, for two reasons:
	//
	//   - It is exactly what Discord keys the MESSAGE_CONTENT privileged-
	//     intent exemption on ("content in messages in which the app is
	//     mentioned") — so testing this array for our bot's id makes "we
	//     consider ourselves addressed" and "Discord actually gave us the
	//     content" the same condition, instead of two conditions (a regex
	//     over Content vs. Discord's own resolution) that can disagree.
	//   - It captures cases a content regex cannot: when a user replies to
	//     a message with Discord's default "ping the replied-to author"
	//     behavior enabled, Discord adds that author to mentions WITHOUT
	//     inserting a "<@id>" token into Content. A regex over Content
	//     alone misses this case entirely (see inbound.go).
	//
	// See inbound.go's mentioned/botInMentionsArray computation for how
	// this is combined with the content-regex fallback.
...
func parseMessageCreate(data json.RawMessage) (MessageCreateEvent, error) {
	var raw struct {
		...
		// Mentions is Discord's documented array of user objects resolved
		// as mentioned by this message. Only "id" is decoded; absent or
		// null (never observed, but treated the same as absent) decodes to
		// a nil/empty slice, which callers must treat as "no mentions
		// reported" and NOT as "mentions unknown, assume addressed".
		Mentions []struct {
			ID string `json:"id"`
		} `json:"mentions"`
	...
	if len(raw.Mentions) > 0 {
		evt.Mentions = make([]MentionedUser, len(raw.Mentions))
		for i, m := range raw.Mentions {
			evt.Mentions[i] = MentionedUser{ID: m.ID}
		}
	}
