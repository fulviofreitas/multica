...
vi.mock("@multica/core/api", () => ({
  api: { redeemDiscordBindingToken: mockRedeemToken },
}));

import { DiscordBindPage } from "./bind-page";
...
function renderPage(token: string | null) {
  return render(<DiscordBindPage token={token} />, { wrapper: I18nWrapper });
}

describe("DiscordBindPage", () => {
  beforeEach(() => {
  ...
  it("preserves the token in the login next parameter", () => {
    ...
    expect(decodeURIComponent(destination.split("next=")[1] ?? "")).toBe(
      "/discord/bind?token=token%20with%2B%2Freserved",
    );
  });
  ...
  it("redeems a one-time token exactly once under StrictMode", async () => {
    ...
    render(
      <StrictMode>
        <DiscordBindPage token="single-use-token" />
      </StrictMode>,
      { wrapper: I18nWrapper },
    );
